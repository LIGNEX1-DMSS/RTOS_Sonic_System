...
//            printf("Initial Buffer : %s\n", cmdBuffer);
            ...
            if (cmdBuffer[0] == 'A')
                ...
                strcpy((char*)ackBuf, "ACK_FIRE:B\r\n");

            else if (cmdBuffer[0] == 'N')
                strcpy((char*)ackBuf, "ACK_FIRE:N\r\n");

            else
                strcpy((char*)ackBuf, "NACK_FIRE\r\n");
...
//        printf("Buffer %c\n", &cmdBuffer);
        ...
        HAL_UART_Receive_IT(&huart1, &rxByte, 1);  // 다음 바이트 계속 수신
    }
}
...
//  osKernelStart();
  ...
  char buffer[64];
  int sensorA, sensorB, diff;

  HAL_UART_Receive_IT(&huart1, &rxByte, 1); // 인터럽트 수신 시작
...
//  	  HAL_GPIO_TogglePin(LD3_GPIO_Port, LD3_Pin);
  	  ...
  	  sensorA = rand()%100;
  	  sensorB = rand()%100;
  	  diff = sensorA-sensorB;
  	  sprintf(buffer, "A:%d,B:%d,D:%d\r\n", sensorA, sensorB, diff);
//  	  printf("A:%d,B:%d,D:%d\r\n", sensorA, sensorB, diff);
